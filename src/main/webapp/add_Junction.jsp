...
<body>
  ...
  <div class="py-5" style="	background-image: url(http://wallup.net/wp-content/uploads/2016/01/136660-animals-bears-forest.jpg);	background-position: top left;	background-size: 100%;	background-repeat: repeat;">
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <h1 class="text-light"><b>Add animal's characteristics</b></h1>
        </div>
      </div>
    </div>
  </div>
  <div class="py-1" >
    <div class="container">
      <div class="row">
				<div class="col-md-12">
					<form action="JunctionController" method="GET">
						<table>
							<tr>
								<td style="font-size:1.5vw">Animal:</td>
								<td><select name="idpacient">
										<c:forEach items="${listaPacienti}" var="pacienti">
											<option value="${pacienti.idanimals}">${pacienti.idanimals},
												${pacienti.name}, ${pacienti.region}</option>
										</c:forEach>
								</select></td>
							</tr>
							<tr>
								<td style="font-size:1.5vw">Characteristics:</td>
								<td><select name="idmedic">
										<c:forEach items="${listaMedici}" var="medici">
											<option value="${medici.idcharacteristics}">${medici.idcharacteristics},
												${medici.clas}, ${medici.orders}, ${medici.species},
												${medici.kingdom}</option>
										</c:forEach>
								</select></td>
							</tr>
							<tr>
								<td><input class="btn btn-primary" type="submit" name="adaugaConsultatie"
									value="Bind them" style="font-size:1vw"></td>
							</tr>
						</table>
					</form>
				</div>
				<form action="JunctionController" method="POST">
					<input class="btn btn-primary" type="submit" name="afiseazaConsultatie"
						value="Show animals' characteristics database" style="font-size:1vw"> &nbsp;
					&nbsp;<br>
				</form>				
			</div>
    </div>
